...
const wrapToRootNodeIfNeeded = text =>
  `${text.substring(0, 6) !== `[${rootTag}]` ? `[${rootTag}]` : ""}${text}${
...
const getAllTagsRecursively = (text, regex, result = [], codeIndex = 0) => {
  ...
  const processTextType = localCodeIndex => {
    if (
      localCodeIndex === 0 ||
      (matchedResult.tag.toLowerCase() === codeTag ||
        matchedResult.tag.toLowerCase() === brTag)
    ) {
      if (
        result.length > 0 &&
        result[result.length - 1].lastIndex !== matchedResult.firstIndex
      ) {
        const calculatedTextPart = {
          type: TEXT,
          match: text.substring(
            result[result.length - 1].lastIndex,
            matchedResult.firstIndex
          ),
  ...
  if (!bbCodesMapNames.includes(matchedResult.tag.toLowerCase())) {
    ...
    matchedResult.type === OPEN_TAG &&
    matchedResult.tag.toLowerCase() === codeTag
  ) {
    const newCodeIndex = codeIndex + 1;
    return getAllTagsRecursively(
      text,
      regex,
      [...result, ...processTextType(newCodeIndex)],
      newCodeIndex
    );

    // if close tag 'code' - codeIndex--
  } else if (
    matchedResult.type === CLOSE_TAG &&
    matchedResult.tag.toLowerCase() === codeTag
  ) {
    const newCodeIndex = codeIndex - 1;
    return getAllTagsRecursively(
  ...
  return getAllTagsRecursively(
    text,
    regex,
    [...result, ...processTextType(codeIndex)],
    codeIndex
  );
};

const buildNodeTreeRecursively = (
  tagsArray,
  tagIndex = 0,
  result = {},
  parentNode = {}
  ...
  if (currentTag) {
    ...
    if (currentTag.type === OPEN_TAG) {
      ...
      currentTag.type === CLOSE_TAG &&
      currentTag.tag === parentNode.type
    ) {
      const nextTagIndex = tagIndex + 1;
      return buildNodeTreeRecursively(
        tagsArray,
        nextTagIndex,
        result,
...
const parseTextToNodeTree = text => {
  ...
  const allTags = getAllTagsRecursively(
    message,
