...
export const GET_TOPIC_ARRAY_BY_FORUM_ID_SUCCESS =
...
export const getTopicArrayForumIdNonBlockSaga = function*({
  forumId,
  pageNumber,
  pageSize
}) {
  const { response, error } = yield call(getTopicArrayByForumIdApi, {
    forumId,
    pageNumber,
    pageSize
  });
...
export const forumSaga = function*() {
  yield all([getTopicArrayForumIdSaga()]);
};
