...
export const chapterArraySelector = state =>
  state.conferenceReducer.chapterArray;
export const lastActiveTopicArraySelector = state =>
  state.conferenceReducer.lastActiveTopicArray;
  ...
  (chapterArray, forumArray) => {
    if (forumArray && forumArray.length > 0) {
      return chapterArray.reduce((previous, current) => {
        const filteredForumArray = forumArray.filter(
          x => x.chapterId === current.id
        );
        const chapterItemWithForumArray = {
          ...current,
          ...{ forumArray: filteredForumArray }
        };
