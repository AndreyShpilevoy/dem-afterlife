...
const getForumBreadcrumbsRecursively = (
  array,
  forumId,
  result = [],
  index = 1000
  ...
  if (selectedForum && selectedForum.parentForumId) {
    return getForumBreadcrumbsRecursively(
      array,
      selectedForum.parentForumId,
      [
        { path: `/Forum/${forumId}`, title: selectedForum.title, order: index },
        ...result
      ],
      index - 1
    );
...
const getTopicBreadcrumbsArrayByTopicIdApi = topicId =>
  new Promise(resolve => {
    const selectedTopic = topicArray.find(x => x.id === topicId);
    const result = getForumBreadcrumbsRecursively(
      forumArray,
      selectedTopic.forumId,
      [{ path: `/Topic/${topicId}`, title: selectedTopic.title, order: 1000 }],
      999
    );
    setTimeout(() => {
