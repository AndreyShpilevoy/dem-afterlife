...
export const getTermTranslation = (term, locale) => {
  const termIsValid =
    term && term.value && Number.isInteger(Number.parseInt(term.id, 10));
  if (termIsValid && locale) {
    const currentTranslations = getCurrentTranslations(locale);
    if (currentTranslations) {
      const translation = currentTranslations.translationArray.find(
        x => x.id === term.id
      );
      if (translation) {
...
export const transformRelativeLessThanMinute = (msDeltaTime, locale) =>
  getTermTranslation({ id: 5, value: "less than a minute" }, locale);

export const transformRelativeMinute = (
  msDeltaTime,
  locale,
  dateTimeDeclension
) => {
  const number = Math.round(msDeltaTime / milliseconds.perMinute);
  const oneMinute = getTermTranslation(
    { id: 6, value: "about a minute" },
    locale
  );
...
export const transformRelativeHour = (
  msDeltaTime,
  locale,
  dateTimeDeclension
) => {
  const number = Math.round(msDeltaTime / milliseconds.perHour);
  const firstForm = getTermTranslation(
    { id: 10, value: "about an hour" },
    locale
  );
...
export const transformRelativeDay = (
  msDeltaTime,
  locale,
  dateTimeDeclension
) => {
...
export const transformRelativeMonth = (
  msDeltaTime,
  locale,
  dateTimeDeclension
) => {
...
export const transformRelativeYear = (
  msDeltaTime,
  locale,
  dateTimeDeclension
) => {
...
export const getRelativeDateTime = (date, locale) => {
  ...
  return `${getTransformForPeriod(msDeltaTime)(
    msDeltaTime,
    locale,
    currentTranslations.dateTimeDeclension
  )} ${suffixAgo}`;
};
