...
describe('QuotePure', () => {
    it('component with options should match expected snapshot', () => {
        expect(shallow(<QuotePure options={'test'} styles={styles}>{'content'}</QuotePure>)).toMatchSnapshot();
    });

    it('component without options should match expected snapshot', () => {
        expect(shallow(<QuotePure styles={styles}>{'content'}</QuotePure>)).toMatchSnapshot();
    });
});