...
const mapPropsNativeProps = props =>
  Object.keys(NativeLink.propTypes).reduce(
    (previous, current) =>
      props[current]
...
export const LinkPure = props => {
  const localClassName = props.className
    ? `${props.classes.link} ${props.className}`
    : props.classes.link;
  return (
    <NativeLink className={localClassName} {...mapPropsNativeProps(props)} />
  );
};
