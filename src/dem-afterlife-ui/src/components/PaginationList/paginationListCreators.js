...
export const generateStartPaginationList = (
...
export const generateMiddlePaginationList = (
  paginationItemsCount,
  pageNumber,
  maxListLength
) =>
  Array.from(Array(paginationItemsCount)).reduce((previous, current, index) => {
    ...
    if (currentPageNumber === 1) {
      ...
      currentPageNumber >= pageNumber - offset &&
      currentPageNumber <= pageNumber + offset
    ) {
      return [
        ...previous,
...
export const generateEndPaginationList = (
  paginationItemsCount,
  pageNumber,
  maxListLength,
  offsetListLength
  ...
  Array.from(Array(paginationItemsCount)).reduce((previous, current, index) => {
    ...
    if (currentPageNumber === 1) {
      ...
      currentPageNumber >=
      paginationItemsCount - offsetListLength - 1
    ) {
      return [
        ...previous,
        {
