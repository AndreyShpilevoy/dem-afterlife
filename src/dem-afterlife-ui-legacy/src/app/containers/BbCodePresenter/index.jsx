...
const mapNodeToComponent = (node, classes) => {
    const {content, key, type, options, children} = {...node, options: {value: node.options, classes} };
    const Component = getComponentByTagName(type);

    if (Component) {
        if (children.length > 0) {
            const childrenMappedToComponent = children.map(x => mapNodeToComponent(x, classes));
...
export const BbCodePresenterPure = ({text, classes}) => mapTextToComponentsTree(text, classes);
...
export default injectSheet(calculateStyles, {componentName: 'BbCodePresenter'})(BbCodePresenterPure);